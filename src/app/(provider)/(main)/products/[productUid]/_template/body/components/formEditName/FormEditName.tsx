...
const FormEditName: React.FC<FormProps> = ({ visible, uid, onCancel, onSuccess }) => {
  const [form] = Form.useForm<FormData>();
  const [nameToCheck, setNameToCheck] = useState<string>("");

  // Mutation để tạo item mới
  const createItemMutation = useMutation({
    mutationFn: (data: FormData) => ProductClient.updateName(uid, data.name),
    onSuccess: () => {
      getMessageApi().success("Sửa tên sản phẩm thành công!");
      form.resetFields();
      setNameToCheck("");
      onSuccess();
    },
    onError: (error: any) => {
      getMessageApi().error(error?.message || "Có lỗi xảy ra khi sửa tên sản phẩm");
    },
  });
  ...
  return (
    <Modal title="Sửa tên sản phẩm" open={visible} onCancel={handleCancel} footer={null} width={500} destroyOnHidden>
