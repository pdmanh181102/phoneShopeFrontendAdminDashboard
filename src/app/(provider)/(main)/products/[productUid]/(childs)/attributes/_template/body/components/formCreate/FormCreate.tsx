...
const FormCreate: React.FC<FormProps> = ({ productUid, visible, onCancel, onSuccess }) => {
  const [form] = Form.useForm<FormData>();
  const [nameToCheck, setNameToCheck] = useState<string>("");

  // Mutation để tạo item mới
  const createItemMutation = useMutation({
    mutationFn: (data: FormData) => ProductAttributeClient.create(productUid, data),
    onSuccess: () => {
      getMessageApi().success("Thêm thuộc tính sản phẩm thành công!");
      form.resetFields();
      setNameToCheck("");
      onSuccess();
    },
    onError: (error: any) => {
      getMessageApi().error(error?.message || "Có lỗi xảy ra khi thêm thuộc tính sản phẩm");
    },
  });

  // Query để check tên item có tồn tại không với debounce
  const { data: isNameExists, isFetching: isCheckingName } = useQuery({
    queryKey: ["check-product-attribute", productUid, nameToCheck],
  ...
  return (
    <Modal title="Thêm thuộc tính sản phẩm mới" open={visible} onCancel={handleCancel} footer={null} width={500} destroyOnHidden>
      <Form form={form} layout="vertical" onFinish={handleSubmit} autoComplete="off">
        ...
        >
          <Input autoFocus={true} placeholder="Nhập tên thuộc tính sản phẩm" onChange={handleNameChange} maxLength={100} showCount />
        </Form.Item>
