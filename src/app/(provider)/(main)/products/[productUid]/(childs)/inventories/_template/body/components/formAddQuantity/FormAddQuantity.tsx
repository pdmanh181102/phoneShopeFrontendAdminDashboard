...
const FormAddQuantity: React.FC<FormProps> = ({ visible, productUid, onCancel, onSuccess }) => {
  const [form] = Form.useForm<FormData>();

  // Mutation để tạo item mới
  const createItemMutation = useMutation({
    mutationFn: (data: FormData) => ProductInventoryClient.updateAddQuantity(productUid, data.number),
    onSuccess: () => {
  ...
  return (
    <Modal title="Thêm sản phẩm" open={visible} onCancel={handleCancel} footer={null} width={500} destroyOnHidden>
